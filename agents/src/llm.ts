      ...
      (res) => {
        ...
        res.on("end", () => {
          ...
          try {
            ...
            if (!content) {
              reject(new Error(`DeepSeek empty content (finish_reason=${j.choices?.[0]?.finish_reason})`));
              return;
            }
            resolve(content);
          } catch (e) {
...
function viaMock(intake: {
  ...
  if (intake.amountUsd > 100000) (risk += 15), flags.push("large ticket");
  ...
  return {
    opinion: {
      ...
      rationale: `Deterministic heuristic scoring: tenor ${days.toFixed(0)}d, ticket $${intake.amountUsd}, flags: ${flags.join(", ") || "none"}.`,
      red_flags: flags,
      confidence: 0.55,
    },
    provider: "mock",
